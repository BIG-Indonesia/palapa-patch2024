...
function olAddRESTLayer(serviceUrl, id,p_name_esri) {
...
$('#ext_wms_item_list').on('click', function(e) {
    ...
    if (tipe == 'WMS') {
		...
		//Modif by YSN
		p_name_esri = $(e.target).find('.layermark').first().text();
		if (p_name_esri == '' || typeof(p_name_esri) == 'undefined') {
            p_name_esri = $(e.target).closest('.layermark').first().text();
            if (p_name_esri == '' || typeof(p_name_esri) == 'undefined') {
                p_name_esri = $(e.target).siblings('.layermark').first().text();
            }
        }
		olAddRESTLayer(srv_url, p_id,p_name_esri);
    }
})
                    ...
                    $.ajax({
                            ...
                            success: function(data) {
                                ...
                                for (i = 0; i < layers.length; i++) {
                                    //item_html = "<li id='" + layers[i].id + "' class='collection-item'><i id='add_check' class='material-icons ilist2'>add_circle</i> <span class='layermark lilist2' id='" + layers[i].id + "'>" + layers[i].name + "</span></li>";
                                    //Modif By Yudha.S.N.
									item_html = "<li id='" + layers[i].id + "' class='collection-item'><i id='add_check' class='material-icons ilist2'>add_circle</i> <span class='layermark lilist2' id='" + layers[i].name + "'>" + layers[i].name + "</span></li>";
